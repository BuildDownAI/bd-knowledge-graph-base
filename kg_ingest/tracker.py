...
def _classify(body: str, *, headings: tuple = (), is_bot: bool = False) -> str | None:
    ...
    # Implementation note: bot-only — first substantive line contains
    # "Implementation Summary", or body opens with "## Summary" followed by
    # an "## Approach" or "## Test plan" heading (the two-path rule from KGB-24
    # planning: _first_line skips bare one-word headings like "## Summary").
    if is_bot:
        fl = _first_line(body)
        if "Implementation Summary" in fl:
            return "implementation_note"
        stripped = body.lstrip()
        if stripped.startswith("## Summary") and re.search(
            r"^## (Approach|Test plan)", body, re.MULTILINE
        ):
            return "implementation_note"
...
def _comment_title(ident: str, issue_title: str, body: str, kind: str) -> str:
    ...
    if kind == "implementation_note":
        return f"{ident} implementation summary"
    fl = _first_line(body)
...
def _add_issue(spine_g: Graph, run_g: Graph, run: URIRef, team: str,
    ...
    for i, cm in enumerate(((iss.get("comments") or {}).get("nodes", []))):
        ...
        _KIND_CLS = {
            ...
            "implementation_note": KG.ImplementationNote,
        }
        ...
        if kind in ("learning", "planning_note", "verification", "implementation_note"):
        ...
        stat_key = {
            ...
            "implementation_note": "comment_implementation_notes",
        }[kind]
        stats[stat_key] += 1
...
def add_tracker(spine_g: Graph, run_g: Graph, repo_slug: str, run_id: str,
    ...
    stats = {"teams": [], "issues": 0, "comment_learnings": 0,
             ...
             "comment_verifications": 0, "comment_implementation_notes": 0}
